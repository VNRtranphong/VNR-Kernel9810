...
function FUNC_BUILD_KERNELN9
{
	...
	mv $RDIR/arch/$ARCH/boot/dtb.img $RDIR/arch/$ARCH/boot/boot.img-dtb
			...
			rm -f $RDIR/ramdisk/N960/split_img/boot.img-dtb
			mv -f $RDIR/arch/$ARCH/boot/boot.img-zImage $RDIR/ramdisk/N960/split_img/boot.img-zImage
			mv -f $RDIR/arch/$ARCH/boot/boot.img-dtb $RDIR/ramdisk/N960/split_img/boot.img-dtb
...
function FUNC_BUILD_KERNELS9
{
	...
	echo -e "${red}"; echo -e "${blink_red}"; echo "Make IMG"; echo -e "${restore}";
